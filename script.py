...
def create_pbns(start_path) -> list[PBN]:
...
def create_tasks_and_resources(
    PBNs: list[PBN], AIRFLOW_VAR_BI_OPERATIONS_SALESFORCE_PARAMETERS=""
):
                    ...
                    if isinstance(node, ast.Assign):
                        ...
                        for target in node.targets:
                            ...
                            if isinstance(target, ast.Name):
                                ...
                                if (
                                    "BQ_PROJECT" in target.id
                                    or "BQ_DATASET" in target.id
                                ):
                                    bigquery_parameters[var_name] = (
                                        AIRFLOW_VAR_BI_OPERATIONS_SALESFORCE_PARAMETERS[
                                            re.findall(regexp, ast.unparse(node.value))[
                                                0
                                            ]
                                        ]
                                    )
...
def main():
    ...
    PBNs = create_pbns(start_path=start_path)
    dags, tasks, resources = create_tasks_and_resources(
        PBNs=PBNs,
